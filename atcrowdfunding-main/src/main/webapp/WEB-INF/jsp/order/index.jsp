	...
	<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
		<div class="container-fluid">
			<div class="navbar-header">
				<div>
					<a class="navbar-brand" style="font-size: 32px;" href="#">人人筹 - 项目管理</a>
				</div>
			</div>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<form class="form-inline" id="form" style="float: left;">
							...
							<button type="button" class="btn btn-warning" onclick="queryOrder()">
						...
						<%--<button type="button" class="btn btn-primary"--%>
							<%--style="float: right;" onclick="window.location.href='${APP_PATH}/orderManager/add.htm'">--%>
							...
							<table class="table  table-bordered">
								<thead>
									<tr>
										...
										<th style='font-size: 12px'>发布者id</th>
										<th style='font-size: 12px'>订单金额（元）</th>
										<th style='font-size: 12px'>收获地址</th>
										<th style='font-size: 12px'>收获人</th>
										<th style='font-size: 12px'>收获电话</th>
	...
	<script type="text/javascript">
            ...
            function queryPage( pageIndex ){
            	...
            	$.ajax({
            		...
            		url : "${APP_PATH}/orderManager/pageQuery.do",
            		type : "POST",
            		data : dataObj,
                    dataType:"JSON",
            		beforeSend : function(){
            		...
            		success : function(result){
            			...
            			if(result.info=="success"){
            				...
            				$.each(list,function(i,n){
								...
								content+="	<td style='font-size: 5px'>"+n.ordernum+"</td>";
								content+="	<td style='font-size: 5px'>"+n.memberid+"</td>";
                                content+="	<td>"+n.money+"</td>";
                                content+="	<td style='font-size: 5px'>"+n.address+"</td>";
                                content+="	<td style='font-size: 5px'>"+n.gainusername+"</td>";
                                content+="	<td style='font-size: 5px'>"+n.gainphone+"</td>";
								content+="	<td style='font-size: 5px'>"+n.createdate+"</td>";

								if(n.status=='0'){
									content+="	<td style='font-size: 5px;color: #ec971f'>待付款</td>";
								}else if(n.status=='1'){
									content+="	<td style='font-size: 5px;color: #3e8f3e'>交易完成</td>";
								}else if(n.status=='2'){
									content+="	<td style='font-size: 5px;color: #b92c28'>交易关闭</td>";
								}
								content+="	<td>";
                                <%--//订单不支持修改--%>
                                <%--content+="		<button type='button' onclick='window.location.href=\"${APP_PATH}/orderManager/edit.htm?pageNo="+pageObj.pageNo+"&id="+n.id+"\"' class='btn btn-primary btn-xs'>";--%>
                                <%--content+="			<i class=' glyphicon glyphicon-pencil'></i>";--%>
                                <%--content+="		</button>";--%>
								content+="		<button type='button' class='btn btn-danger btn-xs' onclick='deleteOrder("+n.id+")'>";
            ...
            function deleteOrder(id){
    			...
    			layer.confirm("确认要删除该订单吗?",  {icon: 3, title:'提示'}, function(cindex){
    				
    				$.ajax({
    					url : "${APP_PATH}/orderManager/delete.do",
    					type : "POST",
    					data : {id:id},
    					success : function(result){
    					    console.log(result);
            ...
            $("#batchDelete").click(function(){
            	...
            	if(checkedList.length>0){
            		layer.confirm("确定要删除吗?",  {icon: 3, title:'提示'}, function(cindex){
            			...
            			$.ajax({
            				url : "${APP_PATH}/orderManager/batchDelete.do",
            				type : "POST",
            				data : datas ,
            				success : function(result){
            					if(result.info=="success"){
                    ...
                    layer.msg("请先选中需要删除的订单!", {time:2000, icon:5, shift:6});
                }
            });
