...
<body>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<form class="form-inline" id="form" style="float: left;">
							...
							<button type="button" class="btn btn-warning" onclick="queryProject()">
						...
						<%--<button type="button" class="btn btn-primary"--%>
							<%--style="float: right;" onclick="window.location.href='${APP_PATH}/projectManager/add.htm'">--%>
	...
	<script type="text/javascript">
            ...
            <%--function changepageNo( pageNo ) {--%>
            	<%--//window.location.href = "<%=request.getContextPath() %>/projectManager/index.htm?pageNo="+pageNo;--%>
            	<%--queryPage(pageNo-1);--%>
            <%--}--%>
            ...
            function queryPage( pageIndex ){
            	...
            	$.ajax({
            		...
            		url : "${APP_PATH}/projectManager/pageQuery.do",
            		type : "POST",
            		data : dataObj,
                    dataType:"JSON",
            		beforeSend : function(){
            		...
            		success : function(result){
            			...
            			if(result.info=="success"){
            				...
            				$.each(list,function(i,n){
								...
								if(n.status==0){
									content+="	<td style='font-size: 5px;color: #ec971f'>未审核</td>";
								}else if(n.status==1){
									content+="	<td style='font-size: 5px;color: #3e8f3e'>众筹中</td>";
								}else if(n.status==2){
									content+="	<td style='font-size: 5px;color: #3e8f3e'>众筹成功</td>";
								}else if(n.status==3){
									content+="	<td style='font-size: 5px;color: #ac2925'>众筹失败</td>";
								}else if(n.status==4){
                                    content+="	<td style='font-size: 5px;color: #ac2925'>审核失败</td>";
                                }
								...
								content+="		</button>";
								content+="		<button type='button' class='btn btn-danger btn-xs' onclick='deleteProject("+n.id+")'>";
            ...
            function deleteProject(id){
    			...
    			layer.confirm("确认要删除该项目吗?",  {icon: 3, title:'提示'}, function(cindex){
    				
    				$.ajax({
    					url : "${APP_PATH}/projectManager/delete.do",
    					type : "POST",
    					data : {id:id},
    					success : function(result){
    					    console.log(result);
            ...
            $("#batchDelete").click(function(){
            	...
            	if(checkedList.length>0){
            		layer.confirm("确定要删除吗?",  {icon: 3, title:'提示'}, function(cindex){
            			...
            			$.ajax({
            				url : "${APP_PATH}/projectManager/batchDelete.do",
            				type : "POST",
            				data : datas ,
            				success : function(result){
            					if(result.info=="success"){
                    ...
                    layer.msg("请先选中需要删除的项目!", {time:2000, icon:5, shift:6});
                }
            });
